...
CREATE TABLE localidad (
    identificador SERIAL,
    nombre enum_localidad NOT NULL,
    PRIMARY KEY (identificador)
);
...
INSERT INTO idioma (nombre, nivel, descripcion_nivel, cadena_busqueda)
    SELECT DISTINCT SUBSTRING(nombre_idioma_nivel, '([[:alpha:]]| )+'), 
                    SUBSTRING(nombre_idioma_nivel, '((A|B|C)(1|2)|TOEFL| / |IELTS)+'), 
                    SUBSTRING(nombre_idioma_nivel, 'Nivel ([[:alpha:]]| |[0-9])+'), nombre_idioma_nivel
    FROM acuerdo_movilidad;
...
INSERT INTO area_estudio (nombre, codigo_area)
    SELECT DISTINCT REGEXP_REPLACE(nombre_area_estudio, '([0-9]+|\-|\.|\]|\[| )*',''),
                    REGEXP_REPLACE(nombre_area_estudio, '([[:alpha:]]+|\,| |\-[[:alpha:]]|\)|\(|\- [[:alpha:]])+','') 
    FROM acuerdo_movilidad;
...
INSERT INTO convenio (curso_academico, nombre_programa, in_out, id_centro, id_universidad,  id_idioma, id_area, plazas_ofertadas, plazas_asignadas_out)
    ...
    WHERE p.identificador = ua.id_pais AND
          ...
          ae.nombre IN (SELECT DISTINCT REGEXP_REPLACE(nombre_area_estudio, '([0-9]+|\-|\.|\]|\[| )*','')) AND
          ae.codigo_area IN (SELECT DISTINCT REGEXP_REPLACE(nombre_area_estudio, '([[:alpha:]]+|\,| |\-[[:alpha:]]|\)|\(|\- [[:alpha:]])+',''))
